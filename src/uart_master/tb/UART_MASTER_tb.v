...
module tb () ;
    ...
    UART_MASTER u_UART_MASTER_Top (.I_CLK(I_CLK), .I_RESETN(rst_n), .I_TX_EN(I_TX_EN), .I_WADDR(I_WADDR), .I_WDATA(I_WDATA), .I_RX_EN(I_RX_EN), .I_RADDR(I_RADDR), .O_RDATA(O_RDATA), .DDIS(DDIS), .INTR(INTR), .SIN(rx_o), .RxRDYn(RxRDYn), .SOUT(tx_o), .TxRDYn(TxRDYn), .DCDn(DCDn), 
